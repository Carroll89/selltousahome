#!/usr/bin/env python3
"""Generate TTS audio for all 5 new markets + Erie FAQ retry check."""
import json
import os
...
scripts = json.load(open(os.path.join(OUTPUT_DIR, "new-market-scripts.json")))
...
for name, script in scripts.items():
    ...
    if os.path.exists(audio_path) and os.path.getsize(audio_path) > 10000:
        print(f"SKIP {name} (audio exists, {os.path.getsize(audio_path)//1024}KB)")
        skipped += 1
        continue
...
print(f"\nAudio generation complete: {done} new, {skipped} skipped, {failed} failed")
